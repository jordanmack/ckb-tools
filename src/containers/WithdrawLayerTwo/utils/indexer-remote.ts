...
export class CkbIndexer implements Indexer {
  ...
  constructor(public ckbRpcUrl: string, public ckbIndexerUrl: string) {
    if (!ckbRpcUrl) {
      throw new Error(
        `CkbIndexer has to be constructed with a valid ckbRpcUrl (string). You passed: ${ckbRpcUrl}`
      );
    }

    if (!ckbIndexerUrl) {
      throw new Error(
        `CkbIndexer has to be constructed with a valid ckbRpcUrl (string). You passed: ${ckbIndexerUrl}`
      );
    }
  ...
  collector(queries: QueryOptions): CellCollector {
    ...
    if (lock !== undefined) {
      searchKey = {
        ...
        filter: {
          block_range: ["0x34ffff", "0x6cf8719ffd"]
        }
      };
